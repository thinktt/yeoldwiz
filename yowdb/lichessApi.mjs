...
// let tokens
// if (config.tokens) {
//   tokens = JSON.parse(config.tokens)
...
//   config.tokens = JSON.stringify(tokenToSet)
//   tokens = tokenToSet
// }
...
async function getGames(user, lastGameTime) {
  ...
  const tokens = JSON.parse(config.tokens)
...
async function getGamesByIds(ids) {
  ...
  return async function () {
    ...
    const outs = buffer.split('\n')
    buffer = outs.pop()  // Save incomplete part in the buffer

    for (const out of outs) {
      if (out !== '') return JSON.parse(out)  // Return next complete JSON
    }
  }
}
...
async function startStream(endpoint, callback, onDone) {
  const tokens = JSON.parse(window.config.tokens)
  const controller = new AbortController()
  const signal = controller.signal
  const headers =  {
    'Authorization' : 'Bearer ' + token,
